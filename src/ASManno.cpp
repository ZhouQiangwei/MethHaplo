	...
	if (argc >1) 
	{
		...
		for(int f=InFileStart;f<=InFileEnd;f++)
		{
			...
			for(int f=FileS;f<=FileE;f++){
				...
				{
					if(i==0 || i==2)
					    Mstep = floor((double(distance))*binspan);
					else Mstep = floor((double)(MLEN/ngene)*binspan);
					for(int j=0;j<nLevel;j++)
					{
						fprintf(MethGffOUTFILE,"\t%f",(double) methGff_List[i].C_Num[j]/Mstep);
					}
				}

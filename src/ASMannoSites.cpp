...
int main(int argc, char* argv[])
{
	...
	int BedInFileStart=0,BedInFileEnd=0;
        bool InputPed=false;
        int PedInFileStart=0,PedInFileEnd=0;
	int distance=2000;
	...
	for(int i=1;i<argc;i++)
	{
		...
		else if(!strcmp(argv[i], "--ped"))
                {
                        InputPed=true;
                        PedInFileStart=++i;
                        while(i!=(argc-1) && argv[i][0]!='-'){i++;continue;}
                        if(argv[i][0]=='-') {PedInFileEnd=--i;}else {PedInFileEnd=i ;}
                }
		else
		{
			printf("%s \n",Help_String);
		...
		for(int f=InFileStart;f<=InFileEnd;f++)
		{
			...
			}else if(InputBed)
			{
				...
				FileE=BedInFileEnd;
                        ...
                        {
                                FileS=PedInFileStart;
                                FileE=PedInFileEnd;
                        }
			else
			{
				fprintf(stderr,"\nNo input Gff/Bed/p file .\n");
				exit(0);
			}
			for(int f=FileS;f<=FileE;f++){
				...
				printf("\nProcessing %d out of %d. InFile: %s\n", f-FileS+1,FileE-FileS+1, argv[f]);
					...
					if(filelen>3 && argv[f][filelen-4]=='b' && argv[f][filelen-3]=='e' && argv[f][filelen-2]=='d' && argv[f][filelen-1]=='4' ) 
						bed4 = true;
					if(filelen>3 && argv[f][filelen-4]=='p' && argv[f][filelen-3]=='e' && argv[f][filelen-2]=='d' && argv[f][filelen-1]=='3' )
                                                ped3 = true;
				...
				while(!feof(GFFINFILE))
				{
						...
						if(bed4){
							...
							if(showbed == 0){
								...
								showbed = 1;
                                        ...
                                        {
                                                if(ped3){
                                                        sscanf(Gff,"%s\t%u\t%c",Chrom,&start,&Strand);end=start;
                                                        if(showbed == 0)
                                                        {
                                                                fprintf(stderr, "bed format and defined strand %c\n", Strand);
                                                                showbed = 1;
                                                        ...
                                                        sscanf(Gff,"%s\t%u",Chrom,&start);end=start;
                                                        Strand='.';
                                                        if(showbed == 0){
                                                                fprintf(stderr, "bed format without strand, if you want define strand, please use bed4 format.\n");
                                                                showbed = 1;
					...
					//if(!InputPed && end-start<beddistance) continue;
					int tmp=0;
					if(!InputPed)
						tmp=(start+end)/2+0.5;
					else
						tmp=start;
//					if(!InputPed && !InputBed && end-start < beddistance) fprintf(stderr, "Waring: element %s is too shoter to caculate heatmap.\n", id.c_str());
//					else if(!InputPed && end-start < beddistance) fprintf(stderr, "Waring: element %s:%d-%d is too shoter to caculate heatmap.\n", Chrom, start, end);
					...
					float binspan_b = binspan;
                                        start=tmp-distance; end=tmp+distance;
					if(start>distance && end+distance <Genome_Offsets[H+1].Offset && end-start > nLevel ) //&& end-start > nLevel 
					{
						start--;end--;
						//body
						caculate(start,end,Methy_List[H],Strand,methGff_List[0]);
						if(Strand!='-' && GENE){
							binspan = binspan_b*2;
//							if(!InputPed) caculateHeatmap("GENE",start,end,Methy_List[H],Strand,id.c_str(),MethGff_GENE_OUTFILEcg,Chrom,start,end, MethGff_TSS_OUTFILEcg_matrix,false);
							binspan = binspan_b;
						}
						//
						if(Strand=='-' && GENE) {

...
int main(int argc, char* argv[])
{
	...
	printf("\nMethyhaplo: BEDanno v1.0\n");
	const char* Help_String="Command Format : BEDanno [options] -o <OUT_PREFIX> -G GENOME -gff1 <GFF file>/-gtf1 <GTF file>/-b1 <bed file> -gff2 <GFF file>/-gtf2 <GTF file>/-b2 <bed file> [-B] [-P]\n"
		"\nUsage:\n"
		"\t-o|--out         Output file prefix\n"
		"\t-G|--genome      Genome\n"
		"\t-gtf1|-gff1      Gtf/gff file\n"
		"\t-b1|--BED1       Bed file, chrom start end (strand)\n"
		"\t-gtf2|-gff2      Gtf/gff file\n"
                "\t-b2              Bed file, chrom start end (strand)\n"
	...
	for(int i=1;i<argc;i++)
	{
		...
		else if(!strcmp(argv[i], "-b2"))
		{
			InputBed2=true;
                        BedInFileStart2=++i
		}else if(!strcmp(argv[i], "-gtf2") || !strcmp(argv[i], "-gff2"))
                {
			InputGff2=true;
			if(!strcmp(argv[i], "-gtf2")) GTF2=true;
			GffInFileStart2=++i;
		}else if(!strcmp(argv[i], "-gtf1") || !strcmp(argv[i], "-gff1"))
		{
			InputGff1=true;
			if(!strcmp(argv[i], "-gtf1")) GTF1=true;
			GffInFileStart1=++i;
	...
	if (argc >1) 
	{
		...
		for(int f=InFileStart;f<=InFileEnd;f++)
		{
			...
			while (!feof(INFILE)) 
			{
				...
				sscanf(Meth,"%s%u%u",Chrom,&pos1,&pos2);
			...
			for ( int i=0;i<3;i++)
			{//0--UP 1--BODY 2--DOWN
				methGff_List[i].C_Num=new long[nLevel];
				for(int j=0;j<nLevel;j++)
					methGff_List[i].C_Num[j] = 0;
				methGff_List[i].AverC=0;
			...
			for(int f=FileS;f<=FileE;f++){
				...
				while(!feof(GFFINFILE))
				{
					...
					if(start>distance && end+distance <Genome_Offsets[H+1].Offset && end-start > nLevel ) //&& end-start > nLevel 
					{
						...
						{//Down
							...
							if(TTS)
							{
								caculateHeatmap("TTS",start-distanceHeatmap,start+distanceHeatmap,Methy_List[H],Strand,id.c_str(),MethGff_TTS_OUTFILEcg,Chrom,start,end, MethGff_TSS_OUTFILEcg_matrix);
							}
						}else 
						{//Up
							...
							if(TSS)
							{
								caculateHeatmap("TSS",start-distanceHeatmap,start+distanceHeatmap,Methy_List[H],Strand,id.c_str(),MethGff_TSS_OUTFILEcg,Chrom,start,end, MethGff_TSS_OUTFILEcg_matrix);
							}
							if(GENE) {
								binspan = binspan_b*2;
								caculateHeatmap("GENE",start-distanceHeatmap,start,Methy_List[H],Strand,id.c_str(),MethGff_GENE_OUTFILEcg,Chrom,start,end, MethGff_TSS_OUTFILEcg_matrix,true);
								binspan = binspan_b;
							}
						}
						//body
						caculate(start,end,Methy_List[H],Strand,methGff_List[1]);
						if(Strand!='-' && GENE){
							binspan = binspan_b*2;
							caculateHeatmap("GENE",start,end,Methy_List[H],Strand,id.c_str(),MethGff_GENE_OUTFILEcg,Chrom,start,end, MethGff_TSS_OUTFILEcg_matrix,false);
							binspan = binspan_b;
						}
						//
						if(Strand=='-')
						{
							...
							if(TSS)
							{
								caculateHeatmap("TSS",end-distanceHeatmap,end+distanceHeatmap,Methy_List[H],Strand,id.c_str(),MethGff_TSS_OUTFILEcg,Chrom,start,end, MethGff_TSS_OUTFILEcg_matrix);
							}
							if(GENE) {
								binspan = binspan_b*2;
								caculateHeatmap("GENE",end,end+distanceHeatmap,Methy_List[H],Strand,id.c_str(),MethGff_GENE_OUTFILEcg,Chrom, start,end, MethGff_TSS_OUTFILEcg_matrix, true);
								binspan = binspan_b;
							}
						}else
						{//down
							...
							if(TTS)
							{
								caculateHeatmap("TTS",end-distanceHeatmap,end+distanceHeatmap,Methy_List[H],Strand,id.c_str(),MethGff_TTS_OUTFILEcg,Chrom,start,end, MethGff_TSS_OUTFILEcg_matrix);
							}
							if(GENE) {
								binspan = binspan_b*2;
								caculateHeatmap("GENE",end, end+distanceHeatmap,Methy_List[H],Strand,id.c_str(),MethGff_GENE_OUTFILEcg,Chrom,start,end, MethGff_TSS_OUTFILEcg_matrix,false);
								binspan = binspan_b;
							}
						}
						if(Strand=='-' && GENE) {
							...
							caculateHeatmap("GENE",start,end,Methy_List[H],Strand,id.c_str(),MethGff_GENE_OUTFILEcg,Chrom,start,end, MethGff_TSS_OUTFILEcg_matrix,false);
							//binspan = binspan_b;
							caculateHeatmap("GENE",start-distanceHeatmap,start,Methy_List[H],Strand,id.c_str(),MethGff_GENE_OUTFILEcg,Chrom,start,end, MethGff_TSS_OUTFILEcg_matrix,false);
							binspan = binspan_b;
						}
						if(GENE) {
					...
					for(int i=0;i<3;i++)
					{
						if(i==0 && PU)//Promoter
						{
							if(methGff_List[i].AverC>0) fprintf(MethGffpromoterOUTFILEcg,"%s\t%d\t%c\t%d\t%s\n",Chrom,start+1,Strand,methGff_List[i].AverC,id.c_str());
						}
						if(i==1 && Diff)//body
						{
							if(methGff_List[i].AverC>0 ) fprintf(MethGffbodyOUTFILEcg,"%s\t%d\t%c\t%d\t%s\n",Chrom,start+1,Strand,methGff_List[i].AverC,id.c_str());
						}
						if(methGff_List[i].AverC>0) fprintf(MethGffAverOUTFILE,"%s\t%d\t%s\n",du[i].c_str(),methGff_List[i].AverC,id.c_str());
					}
					//
				}
				...
				for(int i=0;i<3;i++)
				{
					for(int j=0;j<nLevel;j++)
					{
						fprintf(MethGffOUTFILE,"\t%d",methGff_List[i].C_Num[j]);
					}
				}

      ...
      try
		{
			...
			while (fgets(Genome_Offsets[Genome_Count].Genome,39,Location_File)!=0 && Genome_Count<=Genome_CountX)
			{
				...
				if(longestChr < Genome_Offsets[Genome_Count].Offset) longestChr = atoi(Genome_Offsets[Genome_Count].Genome);
...
//delete
			fprintf(stderr, "Done and release memory!\n");
			if(RELESEM){
		...
		time(&End_Time);fprintf(stderr, "Time Taken  - %.0lf Seconds ..\n ",difftime(End_Time,Start_Time));
...
char* process_cigar(const char* cig,int Read_Len)
{
	...
	while(*cig!='\0')
	{
		...
		{
			fprintf(stderr, " --%d%c-- ",atoi(temp),*cig);
    		continue;//break;
		}
	}
	delete []cigar_rm;
...
int conutMismatch(string CIGr, int chrLen, char* Genome_seq, string readString, int pos, int hitType)
{
	...
	unsigned n=0;
	int Nmismatch=0;  //chrLen; //Genome_Offsets[Hash_Index+1].Offset
...
void *Process_read(void *arg)
{
	...
	while( (!bamformat && (samaddress = fgets(s2t,BATBUF,((ARGS *)arg)->samINFILE))!=NULL) || (bamformat && r>0 ))
	{
		...
		unsigned G_Skip=Genome_List[H].Genome-((ARGS *)arg)->Org_Genome;
            int Flag_rm=0;
			...
			while(*cigr!='\0')//RLens--READs Length \\ lens--raw reads length \\ GLens--genome Lens
			{
				...
				}else if(*cigr=='M')
				{
					...
					for(int k=lens,r=RLens,g=Glens;k<length+lens;r++,k++,g++)
					{
						...
						char genome_Char = toupper(Genome_List[H].Genome[pos+g-1]);//
						char genome_CharFor1 = toupper(Genome_List[H].Genome[pos+g+1-1]);
						char genome_CharFor2 = toupper(Genome_List[H].Genome[pos+g+2-1]);
						char genome_CharBac1,genome_CharBac2;
						if(pos+g-1 > 2)
						{
							genome_CharBac1 = toupper(Genome_List[H].Genome[pos+g-1-1]);
							genome_CharBac2 = toupper(Genome_List[H].Genome[pos+g-2-1]);
						}
						if (hitType==1 || hitType==3) {
